...
void look_at_lh(vec3 eye, vec3 center, vec3 up, mat4 out_view_matrix)
{
    ...
    vec3 camera_up;
    glm_vec3_cross(forward, right, camera_up);
    ...
    out_view_matrix[0][1] = camera_up[0];
    out_view_matrix[1][1] = camera_up[1];
    out_view_matrix[2][1] = camera_up[2];
    ...
    out_view_matrix[3][1] = -glm_vec3_dot(camera_up, eye);
